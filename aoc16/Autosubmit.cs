...
namespace aoc16
{
    public class Autosubmit
    {
        ...
        public static Result Submit(int day, int part, string answer, HttpClient httpClient, string resultsFile, Action<int> delayFunc)
        {
            var existingResults = Toml.ToModel(File.Exists(resultsFile) ? File.ReadAllText(resultsFile) : "");
            var key = $"day{day}part{part}";
            if (!existingResults.ContainsKey(key))
            {
                existingResults.Add(key, new TomlTable());
            }
            ...
            if (problemResults.ContainsKey("accepted"))
            {
                ...
                Console.WriteLine($"an answer has already been accepted: {acceptedAnswer}", ColorTranslator.FromHtml("#bdbdbd"));
            ...
            if (resultFromBounds is not null)
            {
                return (Result)resultFromBounds;
            }
            if (problemResults.ContainsKey("rejected"))
            {
                var rejectedResults = (TomlArray)problemResults["rejected"];
                if (rejectedResults.Where(x => (string?)x == answer).Any())
                {
        ...
        private static (Result?, long?, long?) AnswerBounds(string answer, TomlTable problemResults)
        {
            ...
            if (problemResults.ContainsKey("upper_bound"))
            {
                ...
                {
                    Console.WriteLine($"exceeded previous upper bound of {upperBoundResult}", ColorTranslator.FromHtml("#bdbdbd"));
            ...
            if (problemResults.ContainsKey("lower_bound"))
            {
                ...
                {
                    Console.WriteLine($"not reached previous lower bound of {lowerBoundResult}", ColorTranslator.FromHtml("#bdbdbd"));
        ...
        private static Result SubmitToServer(int day, int part, string answer, HttpClient httpClient, Action<int> delayFunc)
        {
            var url = $"https://adventofcode.com/2016/day/{day}/answer";
            ...
            if (response.Contains("You gave an answer too recently")) {
                Match m = new Regex("You have (?:(\\d+)m )?(\\d+)s left to wait").Match(response);
                if (!m.Success)
                {
            ...
            if (response.Contains("your answer is too high"))
            {
                return Result.REJECTED_TOO_HIGH;
            }
            if (response.Contains("your answer is too low"))
            {
