...
{
  private ImmutableList<(ImmutableList<string> outside, ImmutableList<string> inside)> parsed = [];

  public void Presolve(string input)
  {
    var lines = input.Trim().Split("\n");
    parsed = lines.Select(line =>
    {
      var m = AddressRegex().Match(line);
      if (!(m?.Success ?? false)) throw new InvalidDataException($"Invalid input: {line}");
      var outside = m.Groups[1].Captures.Union(m.Groups[3].Captures).Select(c => c.Value)
        .ToImmutableList();
  ...
  {
    return parsed.Where(pair => pair.outside.Any(HasAbba) && !pair.inside.Any(HasAbba)).Count()
      .ToString();
  }
    ...
    return parsed.Select(line =>
      {
        ...
        return (abas, babs);
      }).Where(item => item.abas.Any(aba => item.babs.Contains($"{aba[1]}{aba[0]}{aba[1]}")))
      .Count()
      .ToString();
  }
