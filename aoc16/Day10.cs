...
namespace aoc16
{
    ...
    public partial class Day10 : Solver
    {
        ...
        enum OutputType { Bot, Output }
        record ValueNode(int Value, int Bot);
        record BotOutput(OutputType Type, int Id);
        record BotNode(BotOutput Low, BotOutput High);

        ImmutableList<(int, int)> values = [];
        ImmutableDictionary<int, BotNode> bots = ImmutableDictionary<int, BotNode>.Empty;
        ...
        public void Presolve(string input)
        {
            ...
            foreach (var line in lines) {
                if (ValueRegex().Match(line) is { Success: true } valueMatch) {
                    valuesBuilder.Add((int.Parse(valueMatch.Groups[1].Value), int.Parse(valueMatch.Groups[2].Value)));
                    continue;
                }
                if (BotRegex().Match(line) is { Success: true } botMatch) {
                    ...
                    botsBuilder[botId] = new BotNode(new BotOutput(lowType, lowId), new BotOutput(highType, highId));
                    continue;
                }
        ...
        {
            ImmutableList<(int, int)> botInputs = values;
            while (!botInputs.IsEmpty) {
                ...
                foreach (var input in botInputs.GroupBy(x => x.Item2)) {
                    ...
                    if (inputs.Item1 > inputs.Item2) {
                        inputs = (inputs.Item2, inputs.Item1);
                    }
                    if (inputs == (17, 61)) {
